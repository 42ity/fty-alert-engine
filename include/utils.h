 ...
 *
 * Applicable only to maps that have key (i.e. first templated parameter) convertible to std::string or basic arithmetic
 * type
 *
...
{
    static_assert(std::is_convertible<K, std::string>::value || std::is_arithmetic<K>::value,
...
/*!
 \brief Version of join(const char **str_arr, uint32_t length, const char *separator)" that works until NULL terminating
