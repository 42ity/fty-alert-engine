...
TEST_CASE("engine_server agent")
{
    ...
    // Test case #2.0: add new rule simplethreshold3
    {
    ...
    // Test case #2.1: add new rule
    {
        ...
        // Test case #5: generate alert - low critical (simplethreshold/ abc@fff)
    ...
    // Test case #3: list rules
    {
        ...
        zmsg_addstrf(command, "%s", "LIST");
        zmsg_addstrf(command, "%s", "all");
        zmsg_addstrf(command, "%s", ""); //rule_class (all)
        mlm_client_sendto(ui, "fty-alert-engine", SUBJECT_RULES_RFC, NULL, 1000, &command);

        zmsg_t* recv = mlm_client_recv(ui);

        REQUIRE(zmsg_size(recv) == 6); //header ("LIST"/"all"/"") + 3 rules (simplethreshold3, simplethreshold, simplethreshold2)
        foo = zmsg_popstr(recv);
        REQUIRE(streq(foo, "LIST"));
        zstr_free(&foo);
        foo = zmsg_popstr(recv);
    ...
    // Test case #4: list rules - not yet stored type
    {
        ...
        REQUIRE(zmsg_size(recv) == 3); // header (3) + no 'single' rule
        foo = zmsg_popstr(recv);
        REQUIRE(streq(foo, "LIST"));
        zstr_free(&foo);
        foo = zmsg_popstr(recv);
    ...
    // Test case #4.1: list w/o rules
    {
        ...
        zmsg_addstrf(command, "%s", "LIST");
        zmsg_addstrf(command, "%s", "all");
        zmsg_addstrf(command, "%s", "example class"); //simplethreshold.rule
        mlm_client_sendto(ui, "fty-alert-engine", SUBJECT_RULES_RFC, NULL, 1000, &command);
    ...
    // Test case #5.1: list rules LIST2 (LIST version 2)
    {
        struct {
    ...
    // Test case #5.2: list rules LIST2 (LIST version 2)
    {
        struct {
            std::string payload; // json
    ...
    // #13.1 ADD new rule
    {
        ...
        fty::shm::write_metric("5PX1500-01", "status.ups", "1032.00", "", wanted_ttl);
    }
    ...
    // expected result: SUCCESS
    {
        ...
        zmsg_addstrf(rule, "%s", foo);
        zstr_free(&foo);
        zmsg_addstrf(rule, "%s", "check_update_threshold_simple"); //old_name (to replace)
        mlm_client_sendto(ui, "fty-alert-engine", SUBJECT_RULES_RFC, NULL, 1000, &rule);
    ...
    // Test case #21:   Thresholds imported from devices
    {
        ...
        //      21.1.2  add rule : devicethreshold2 (same name as devicethreshold)
        ...
        zmsg_addstrf(rule, "%s", foo);
        zstr_free(&foo);
        zmsg_addstrf(rule, "%s", "device_threshold_test"); // replace the existing rule (old_name)
        mlm_client_sendto(ui, "fty-alert-engine", SUBJECT_RULES_RFC, NULL, 1000, &rule);
    ...
    // 22-1 : "A40"
    {
        ...
        // 22-2 : threshold si invalid "50AA"
    ...
    // test 23: touch rule, that doesn't exist
    {
        ...
        mlm_client_sendto(ui, "fty-alert-engine", SUBJECT_RULES_RFC, NULL, 1000, &touch_request);
    ...
    // test 24: touch rule that exists
    {
        ...
        // 24.3.1: there exists a RESOLVED alert for this rule; send touch request again
        touch_request = zmsg_new();
        REQUIRE(touch_request);
        zmsg_addstr(touch_request, "TOUCH");
        zmsg_addstr(touch_request, "rule_to_touch");
    ...
    // 25.1 Create a rule we are going to test against; add First rule
    {
        zmsg_t* rule = zmsg_new();
        zmsg_addstrf(rule, "%s", "ADD");
...
#if 1 //
    // works with FTY_PROTO_STREAM_ASSETS/create and seems to have some issues
    // # 26.1 catch message 'create asset', check that we created rules
    {
        ...
        fty_proto_t *brecv = fty_proto_decode (&recv);
        REQUIRE(brecv);
        REQUIRE(fty_proto_ttl (brecv) != uint32_t(-1));
    ...
    // Test case #30: list templates rules
    {
        ...
        zmsg_t* command = zmsg_new();
        zmsg_addstrf(command, "%s", "LIST");
        zmsg_addstrf(command, "%s", "1234"); // correlation ID
        zmsg_addstrf(command, "%s", "all"); // filter
        mlm_client_sendto(ui, "test-autoconfig", SUBJECT_RULES_RFC, NULL, 1000, &command);

        zmsg_t* recv = mlm_client_recv(ui);

        foo = zmsg_popstr(recv);
        REQUIRE(streq(foo, "1234"));
        zstr_free(&foo);
        foo = zmsg_popstr(recv);
        REQUIRE(streq(foo, "LIST"));
        zstr_free(&foo);
